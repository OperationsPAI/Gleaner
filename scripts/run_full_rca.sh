...
uv run --all-packages python scripts/full/platform_cli.py eval batch \
  ...
  -a microrca -a shapleyiq -a nezha \
  --include-sampled \
  -s gleaner -s gleaner_latency_dominate -s gleaner_log_dominate -s gleaner_no_ad -s gleaner_no_dpp \
  -s gleaner_no_logs -s gleaner_no_logs_no_ad -s gleaner_no_rebalance -s gleaner_pure_diversity \
  -s gleaner_top_score -s gleaner_wl_kernel -s random -s tracepicker -s trastrainer -s trastrainer_no_metrics \
  -s sifter -s sieve \
