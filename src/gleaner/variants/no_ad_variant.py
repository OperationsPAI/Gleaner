"""
No Anomaly Detection Variant - Gleaner without alarm system

Features:
- Uses traces, logs, and metrics
- No anomaly detection in quota allocation
- No alarm system activation
"""
...
from ..utils import GleanerConfig


class NoAnomalyDetectionVariant(GleanerSampler):
    """
    Gleaner variant that excludes anomaly detection (alarm system)

    Features:
    - Uses traces, logs
    - No alarm system - always in normal mode
    - Quota allocation without anomaly weighting
    """

    def __call__(self, args: SamplerArgs) -> List[SampleResult]:
        """Execute variant with no anomaly detection"""
        logger.info(f"=== Gleaner No-AD Variant: {args.dataset}/{args.datapack} ===")
        logger.info(f"Mode: {args.mode}, Target rate: {args.sampling_rate}")
        start_time = time.time()

        # Load all data types
        logger.info("Loading data (no anomaly detection)...")
        ...
        # Explicitly disable detector-based biasing in this variant
        # Ensure detector spans do not affect quota allocation
        try:
            self.alarm_system.detector_weight = 1.0
            self.alarm_system.max_detector_budget_ratio = 0.0
            # Keep quota allocator referencing same alarm_system instance
            self.quota_allocator.alarm_system.detector_weight = 1.0
            self.quota_allocator.alarm_system.max_detector_budget_ratio = 0.0
        except Exception:
            pass
        ...
        # Calculate time ranges and QPM rebalancing
        ...
        logger.info(
            f"No-AD variant budget allocation - Warmup factor: {lookback_budget_factor:.3f}, "
            f"Processing factor: {alarm_budget_factor:.3f}, "
            f"QPM rebalancing factor: {qpm_rebalancing_factor:.3f}, "
            f"Target total: {target_total_samples}"
        )

        # Build lookback baselines (but no alarm system activation)
        logger.info("Building lookback baselines (no alarm system)...")
        self.quota_allocator.build_lookback_baselines(
            lookback_encoded, performance_thresholds
        )
        # Note: Don't set alarm system lookback end time - keep alarm inactive
        ...
        if not lookback_encoded.is_empty():
            lookback_sampling_rate = (
                target_total_samples * lookback_budget_factor
            ) / len(lookback_encoded)
            lookback_sampling_rate = min(
                1.0, lookback_sampling_rate * qpm_rebalancing_factor
            )

            logger.info(f"No-AD warmup sampling rate: {lookback_sampling_rate:.3f}")
        ...
        # Process alarm data in batches (without alarm system)
        if not alarm_encoded.is_empty():
            alarm_sampling_rate = (target_total_samples * alarm_budget_factor) / len(
                alarm_encoded
            )
            alarm_sampling_rate = min(1.0, alarm_sampling_rate)

            logger.info(f"No-AD processing sampling rate: {alarm_sampling_rate:.3f}")
            ...
            for batch_idx in range(total_batches):
                ...
                logger.info(
                    f"Processing batch {batch_idx + 1}/{total_batches}: {batch_size} traces (no AD)"
                )

                # Reuse main sampler batch processing (alarm/detector bias disabled above)
        ...
        logger.info(
            f"No-AD variant complete: {len(final_results)}/{len(traces_df)} traces "
