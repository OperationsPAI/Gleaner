"""
No Logs and No Anomaly Detection Variant - Gleaner without logs and alarm system
...
class NoLogsNoADVariant(GleanerSampler):
    """
    Gleaner variant that excludes both logs data and anomaly detection
    ...
    def __call__(self, args: SamplerArgs) -> List[SampleResult]:
        """Execute variant with no logs and no anomaly detection"""
        logger.info(
            f"=== Gleaner No-Logs-No-AD Variant: {args.dataset}/{args.datapack} ==="
        )
        ...
        logger.info("Loading data (no logs, no AD)...")
        data = load_data(
            args.input_folder,
            need_traces=True,
        ...
        logger.info(
            f"No-Logs-No-AD variant budget allocation - Warmup factor: {lookback_budget_factor:.3f}, "
        ...
        logger.info("Building lookback baselines (no logs, no alarm system)...")
        self.quota_allocator.build_lookback_baselines(
            lookback_encoded, performance_thresholds
        )
        # Note: Don't set alarm system warmup end time - keep alarm inactive
        ...
        if not lookback_encoded.is_empty():
            ...
            logger.info(
                f"No-Logs-No-AD warmup sampling rate: {lookback_sampling_rate:.3f}"
            )
        ...
        if not alarm_encoded.is_empty():
            ...
            logger.info(
                f"No-Logs-No-AD processing sampling rate: {alarm_sampling_rate:.3f}"
            )
            ...
            for batch_idx in range(total_batches):
                ...
                logger.info(
                    f"Processing batch {batch_idx + 1}/{total_batches}: {batch_size} traces (no logs, no AD)"
                )
        ...
        logger.info(
            f"No-Logs-No-AD variant complete: {len(final_results)}/{len(traces_df)} traces "
