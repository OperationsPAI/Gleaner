...
```bash
DATA_ROOT=temp/tracepicker-platform uv run python scripts/data/make_tracepicker.py run --src-folder data/tracepicker --dataset-name tracepicker
```
...
## `scripts/data/make_tracepicker.py local-test` Cannot Find Input
...
If you are using a different TracePicker export, either place it under `data/tracepicker/<datapack>/traces_spans.csv` or pass it through `scripts/data/make_tracepicker.py run --src-folder <path>`.
...
The repo intentionally ignores most generated data, output, caches, and virtual environments. If new generated parquet files appear unexpectedly, confirm whether they are generated live inputs, outputs, or optional expected baselines before adding them.
...
- live reduced dataset files under `data/rcabench-platform-v2/` are missing or incomplete.
